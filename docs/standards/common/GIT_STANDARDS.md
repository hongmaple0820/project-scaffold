...
| `codex/*` | Agent 独立任务分支 | 用于隔离 Agent 任务，必须遵守同样验证和合并规则 |
...
脚手架默认采用 GitLab Flow 变体。当前仓库的机器可读配置在 `.scale/workspace.json`：
...
- 常规开发：从最新 `dev` 派生短分支，验证通过后合并或推送到 `dev`。
- 发布生产：只在用户明确要求时，将已验证的 `dev` 快进或合并到生产基线，再 tag 和发布。
- 选择性发布：如果 `dev` 已包含本次不发布的功能，从生产基线创建 `release/vX.Y.Z`，只 cherry-pick 本次发布提交，验证后再合入生产基线。
- 热修复：先在 `dev` 修复；如生产紧急需要，再 cherry-pick 到生产基线、tag、发布，并确认 `dev` 已包含同一修复。
- 工作树生命周期：短分支合并后删除；未推送、未合并或无 upstream 的 worktree 不得自动清理。
